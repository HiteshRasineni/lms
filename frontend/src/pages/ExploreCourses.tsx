...
import { Search, Clock, Users, Star, BookOpen, CheckCircle, Target, Loader2 } from "lucide-react";
import { getCourses, enrollInCourse } from "@/lib/apiService";
  ...
  return (
    <DashboardLayout userRole={user?.role === "Teacher" ? "teacher" : "student"}>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2" data-testid="explore-courses-title">
            Explore Courses
          </h1>
          <p className="text-muted-foreground">Discover and enroll in new courses to expand your knowledge.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredCourses.map((course) => (
              <Card key={course._id} className="hover:shadow-lg transition-shadow" data-testid="course-card">
                <CardHeader>
                  ...
                  <CardDescription>{course.teacher?.name || "Unknown Instructor"}</CardDescription>
                </CardHeader>
      ...
      {selectedCourse && (
        <Dialog open={!!selectedCourse} onOpenChange={() => setSelectedCourse(null)}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto" data-testid="course-preview-modal">
            <DialogHeader>
              <DialogTitle className="text-2xl">{selectedCourse.title}</DialogTitle>
              <DialogDescription>By {selectedCourse.teacher?.name}</DialogDescription>
            </DialogHeader>
              ...
              <TabsContent value="about" className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">{selectedCourse.duration || "N/A"}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-medium">Students Enrolled</p>
                    <p className="text-sm text-muted-foreground">{selectedCourse.enrolledCount || 0}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-medium">Category</p>
                    <p className="text-sm text-muted-foreground">{selectedCourse.category || "General"}</p>
                  </div>
                </div>
                ...
                {selectedCourse.prerequisites && selectedCourse.prerequisites.length > 0 && (
                  <div>
                    ...
                    <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                      {selectedCourse.prerequisites.map((prereq: string, idx: number) => (
                ...
                <div>
                  ...
                  {selectedCourse.learningOutcomes && selectedCourse.learningOutcomes.length > 0 ? (
                    <ul className="space-y-2">
                      {selectedCourse.learningOutcomes.map((outcome: string, idx: number) => (
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="flex items-start gap-2 text-sm">
                        <CheckCircle className="h-4 w-4 text-success mt-0.5" />
                        Apply knowledge through hands-on projects and assignments
                      </p>
                      <p className="flex items-start gap-2 text-sm">
                        <CheckCircle className="h-4 w-4 text-success mt-0.5" />
              ...
              <TabsContent value="reviews" className="space-y-4">
                <div className="flex items-center gap-4 pb-4 border-b">
                  <div className="text-center">
                    <div className="text-3xl font-bold">{selectedCourse.averageRating?.toFixed(1) || "N/A"}</div>
