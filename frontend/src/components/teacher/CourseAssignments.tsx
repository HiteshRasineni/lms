...
export const CourseAssignments = ({ courseId }: CourseAssignmentsProps) => {
  ...
  const loadSubmissions = async (assignmentId: string) => {
    try {
      ...
      // Fetch grades for each submission individually
      const subsWithGrades = await Promise.all(
        subs.map(async (sub: Submission) => {
          try {
            // Try to get the grade for this specific submission
            const gradeResponse = await apiClient.get(
              `/grades/student/${sub.student._id}`
            );
            const grades = gradeResponse.data;

            // Find the grade that matches this submission
            const matchingGrade = grades.find((g: any) => {
              const submissionId = g.submission?._id || g.submission;
              return submissionId.toString() === sub._id.toString();
            });

            return { ...sub, grade: matchingGrade || null };
          } catch (error) {
            // If error fetching grade, just return submission without grade
  ...
  const handleGradeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Submitting grade for submission:", selectedSubmission._id);
      ...
      const response = await apiClient.post(
        `/grades/submission/${selectedSubmission._id}`,
        {
          grade: parseFloat(gradeData.grade),
          feedback: gradeData.feedback,
        }
